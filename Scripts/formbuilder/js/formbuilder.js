...
DEFFLD = {
    ...
    item_checkbox: '<li><input name="CHKED" value="1" class="checkbox" type="checkbox" title="默认选中此项" /><input name="VAL" type="text" class="xs"/><a class="icononly-add iconfont icon-pos" title="添加一个新的选择项">&#xe60b;</a><a class="icononly-del iconfont icon-pos" title="删除此选择项">&#xe619;</a><a class="icononly-mov iconfont icon-pos" title="排序">&#xe63d;</a><a class="icononly-limit iconfont icon-pos" title="添加提交次数限制">&#xe632;</a></li>',
    ...
    item_radio: '<li><input name="CHKED" value="1" class="radio" type="radio" title="默认选中此择项" /><input name="VAL" type="text" class="xs"/><a class="icononly-add iconfont icon-pos" title="添加一个新的选择项">&#xe60b;</a><a class="icononly-del iconfont icon-pos" title="删除此选择项">&#xe619;</a><a class="icononly-mov iconfont icon-pos" title="排序">&#xe63d;</a><a class="icononly-limit iconfont icon-pos" title="添加提交次数限制">&#xe632;</a></li>',
    ...
    name: {
        ...
        json: '({LBL:"关联数据",TYP:"name",REQD:"0",UNIQ:"0",SCU:"pub",FMT:"short",DEF:"",INSTR:"",CSS:"",RELA:{RELA1:{TABLENM:"用户表",FILEDNM:"",RELANM:"",RELATABLE:""},RELA2:{FIELDNM:"",RELANM:""}}})',
        holder: '<li class="field prefocus" style="height:71px;width:97%"></li>'
    },
...
(function () {
    ...
    $("#chkMatch").click(function () {
        //F[IDX].RELA.RELA2.FIELDNM = "";
        //F[IDX].RELA.RELA2.RELANM = "";
        if ($("#chkMatch").is(':checked')) {
            //if (!hasSetRelaField) {
            //    hasSetRelaField = true;

            $("#divMatchContainer").removeClass("hide").addClass("show");
            $("#divAutoCompContainer").removeClass("show").addClass("hide");

            //$("#chkAutoComp").attr("checked", false);

            // 关联数据存入json中
            var tableNM = $("#selMatchFrm").val();
            var fieldNM = $("#selMatchFld").val();
            F[IDX].RELA.RELA1.TABLENM = tableNM;
            F[IDX].RELA.RELA1.FILEDNM = fieldNM;

            // 获取之前已经设置好的关联字段的字段名
            $("#selMatchRELFld").empty();
            $.each(F,function(i, v) {
                if (v.TYP == "name")
                    $("#selMatchRELFld").append("<option>" + v.LBL + "</option>");
            });
        }
        else {
            //if (!hasSetRelaField) {
            //    hasSetRelaField = false;
            //}

            $("#divMatchContainer").removeClass("show").addClass("hide");

            F[IDX].RELA.RELA1.TABLENM = "";
            F[IDX].RELA.RELA1.FILEDNM = "";
        }
    })
    ...
    $("#selMatchRELFld").change(function() {
        var relaNM = $("#selMatchRELFld").val();
        F[IDX].RELA.RELA1.RELANM = relaNM;

        var tableNM = "";
        $.each(F, function(i,v) {
            if (v.TYP == "name" && v.LBL == relaNM) {
                tableNM = v.RELA.RELA1.TABLENM;
            }
        })
        F[IDX].RELA.RELA1.RELATABLE = tableNM;
    });

    //$("#chkAutoComp").click(function () {
    //    M[IDX].RELA.RELA.RELA1.TABLENM = "";
    //    M[IDX].RELA.RELA.RELA1.FILEDNM = "";

    //    if (!hasSetRelaField) {

    //    }

    //    if ($("#chkAutoComp").is(':checked')) {
    //        $("#divAutoCompContainer").removeClass("hide").addClass("show");
    //        $("#divMatchContainer").removeClass("show").addClass("hide");

    //        $("#chkMatch").attr("checked", false);

    //        var fieldNM = $("#selAutoCompSrcFld").val();
    //        var relaNM = $("#selAutoCompFld").val();
    //        M[IDX].RELA.RELA.RELA2.FIELDNM = fieldNM;
    //        M[IDX].RELA.RELA.RELA2.RELANM = relaNM;
    //    }
    //    else {
    //        M[IDX].RELA.RELA.RELA2.FIELDNM = fieldNM;
    //        M[IDX].RELA.RELA.RELA2.RELANM = relaNM;
...
function setPropertieValues(c) {
    ...
    if (c.TYP == "name") {
        // 判断是否是关联数据
        if (c.RELA != "" && c.RELA != null) {
            //$("#chkMatch").trigger("click");
            //$("#divMatchContainer").removeClass("hide").addClass("show");
            //$("#divAutoCompContainer").removeClass("show").addClass("hide");
        }
    }
